...
async def get_all_random_quote(db : Session = Depends(get_db)) -> ResponseQuoteDTO:
    """
    ...
    random_quote = await quote_service.get_all_random_quote(db)
    
    return random_quote

@router.get("/category_random", status_code=200, summary="1개 카테고리 랜덤 쿼트")
async def get_category_random_quote(category_id : int, db : Session = Depends(get_db)) -> ResponseQuoteDTO:
    """
    ...
    random_quote = await quote_service.get_category_random_quote(category_id, db)
...
async def get_category_list_random_quote(category_ids : list[int] = Query(...), db : Session = Depends(get_db)) -> ResponseQuoteDTO:
    ...
    random_quote = await quote_service.get_category_list_random_quote(category_ids, db)
